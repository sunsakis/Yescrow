    ...
    if (hasMetaMask == true) {
      ...
      try {
        if (active) {
          ...
          try {
            await contract.createDepositETH(_seller, {
              value: ethers.utils.parseEther(amount),
            });
          } catch (error) {
            alert(
              "The transaction failed. Please make sure you have enough ETH in your Metamask account and try again."
            );
          }
          try {
            contract.on(
              "NewDepositETH",
              (counter, buyerAddress, sellerAddress, depositAmount, event) => {
                console.log(
